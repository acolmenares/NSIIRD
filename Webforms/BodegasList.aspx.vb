...
Partial Class WebForms_BodegasList
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        If objper_perfil IsNot Nothing Then
            ...
            Rg_Listado.Columns(4).Visible = objper_perfil.Peliminar
            ddl_regional.Enabled = objper_perfil.Pvertodo

            '
            ' Habilitar el eliminar
            '
            Rg_Listado.Columns(4).Visible = objper_perfil.Peliminar
        ...
        If objper_usuario IsNot Nothing Then
            ...
            Rg_Listado.Columns(4).Visible = objper_usuario.Peliminar
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        '
        Dim ListBodegas As List(Of SubTablasBrl) = Session("ListBodegas")

        'si no hay una variable de sesión con la lista.
        If ListBodegas Is Nothing Or Request.QueryString.Item("Refrescar") = 1 Then
            ListBodegas = SubTablasBrl.CargarPorId_TablaPadre(31, ddl_regional.SelectedValue)
            Session("ListBodegas") = ListBodegas
        End If

        Rg_Listado.DataSource = Session("ListBodegas")
        Rg_Listado.DataBind()
    ...
    Protected Sub Rg_Listado_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Rg_Listado.SelectedIndexChanged
        ...
        Response.Redirect("Bodegas.aspx?Editar=1&ID=" & item("id").Text)
    End Sub
    ...
    Protected Sub RadGrid1_DeleteCommand(ByVal source As Object, ByVal e As GridCommandEventArgs) Handles Rg_Listado.DeleteCommand
        Dim productId = DirectCast((DirectCast(e.Item, GridDataItem)).GetDataKeyValue("Id"), Integer)
        Dim objbodega As SubTablasBrl = SubTablasBrl.CargarPorID(productId)
        objbodega.Eliminar()
        ...
        Dim ListBodegas As List(Of SubTablasBrl) = Session("ListBodegas")
        ListBodegas = SubTablasBrl.CargarPorId_TablaPadre(31, objusuario.Sucursales.ID_Enlace)
        Session("ListBodegas") = ListBodegas
        Rg_Listado.DataSource = Session("ListBodegas")
        Rg_Listado.DataBind()
    ...
    Protected Sub btn_limpiar_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btn_limpiar.Click
        Session.Remove("ListBodegas")
        Dim ListBodegas As New List(Of SubTablasBrl)
        ddl_regional.SelectedValue = SeguridadUsuarios.Usuarios.RegionalUsuario(CType(Session("IdUsuario"), Integer))
        ListBodegas = SubTablasBrl.CargarPorId_TablaPadre(31, ddl_regional.SelectedValue)
        Session("ListBodegas") = ListBodegas
        Rg_Listado.DataSource = Session("ListBodegas")
    ...
    Protected Sub btn_Procesar_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btn_Procesar.Click, btn_actualizar.Click
        ...
        If ddl_regional.SelectedValue > 0 Then
            ListBodegas = SubTablasBrl.CargarPorId_TablaPadre(31, ddl_regional.SelectedValue)
        Else
            ListBodegas = SubTablasBrl.CargarPorId_Tabla(31)
        End If
        Session("ListBodegas") = ListBodegas
        Rg_Listado.DataSource = Session("ListBodegas")
        Rg_Listado.DataBind()
